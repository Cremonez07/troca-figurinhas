...
export async function saveSticker(
  ...
  if (stickerCodes.length === 0) {
    return {
      ok: false,
      message: "Nenhum código de figurinha válido encontrado (Ex: BRA10, ARG07)."
    };
  }
  ...
  try {
    ...
    return {
      ok: false,
      message: "Não foi possível salvar as figurinhas. Tente novamente."
    };
  }
}
...
export async function saveProfile(
  ...
  try {
    ...
    return {
      ok: true,
      message: "Perfil atualizado. Agora os colecionadores sabem como combinar a troca."
    };
  } catch (error) {
